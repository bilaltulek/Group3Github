...
OB    plusIconPath.setAttribute('fill', 'currentColor');
...
OB    Sidebar.changeTabTitle(tabName);
...
OB
    addNewAddTaskButton();
OB    addNewTaskButtonEventListener();

    // Task buttons
OB    addCurrentProjectTaskButtons(sidebarButton);
...
OB    for (const button of sidebarButtons) {
      button.addEventListener('click', () => {
OB        changeContent(button, button.dataset.tabName);
      });
    }
  };
OB
...
OB    const addTaskButton = document.querySelector('.editor-add-task-button');
